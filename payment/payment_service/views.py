...
class PaymentMethodView(APIView):
    def get(self, requset):
        serializer = PaymentMethodSerializer(PaymentMethod.objects.all())
...
class PaymentofOrderView(APIView):
    def get(self, request, order_id):
        serializer = PaymentSerializer(Payment.objects.filter(order_id=order_id))
        return Response(serializer.data, status=status.HTTP_200_OK)
