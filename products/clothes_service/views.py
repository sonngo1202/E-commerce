...
from clothes_service.serializers import ClothesInfoSerializer, ProducerSerializer, StyleSerializer, ClothesSerializer, UpdateClothesSerializer
...
class ClothesListView(APIView):
    def get(self, request):
        ...
        serializer = ClothesInfoSerializer(clothes, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
...
class SearchClothesListView(APIView):
    def get(self, request, key):
        clothes = Clothes.objects.filter(Q(title__icontains=key), is_active__in=[True]).all()
        serializer = ClothesInfoSerializer(clothes, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
